...
const getToken=async()=>{
  ...
  setUserToken(token);
    ...
    function showDeliveryAddress(){
      ...
      const acceptOrder = async (orderIdData) => {
     ...
     const rejectOrder = async (orderIdData) => {
      await fetch(
        ...
        <ScrollView
                ...
                contentContainerStyle={styles.articles}>
                    ...
                    <Block flex  >
                        ...
                        ></Fancy>
                        <Fancy 
                        visible={action=="arrived_at_restaurant"} 
                        icon_ios={'ios-pin-outline'} icon_android="md-pin"
                        title={Language.arrived_restaurant} subtitle={Language.arrived_restaurant_info}
                        button={Language.ok} closeAction={()=>{setAction("")}}
                        action={()=>{
                          API.updateOrderStatus(order.id,"arrived_at_restaurant","",(data)=>{
                            setAction("");
                            setOrder(data[0]);
                          })}}
                        ></Fancy>
                        <Fancy 
                        visible={action=="arrived_at_customer"} 
                        icon_ios={'ios-pin-outline'} icon_android="md-pin"
                        title={Language.arrived_customer} subtitle={Language.arrived_customer_info}
                        button={Language.ok} closeAction={()=>{setAction("")}}
                        action={()=>{
                          API.updateOrderStatus(order.id,"arrived_at_customer","",(data)=>{
                            setAction("");
                            setOrder(data[0]);
                          })}}
                        ></Fancy>
                        ...
                        <InfoBox title={Language.summary}>
                            ...
                            <Block row space={"between"} style={{marginTop:16}}>
                                ...
                                <Block><Text bold >{(parseFloat(order.order_price)+parseFloat(order.delivery_price)).toFixed(2)}{config.currencySign}</Text></Block>
                            </Block>
                        </InfoBox>
