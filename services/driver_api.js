 ...
 * 
 */
exports.getDriverStatus=async (callback)=>{APICaller.authAPI('GET','driver/auth/data',{},callback,(error)=>{console.log(error)})};
exports.setActiveStatus=async (status,callback)=>{APICaller.authAPI('GET',"driver/auth/"+(status?"driveronline":"drveroffline"),{},(data)=>{callback(data.working==1)},(error)=>{console.log(error);callback(!status)});};
